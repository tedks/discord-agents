...
- **Claude** — the bot pins it via `--session-id <uuid>`. Fresh sessions start unconfirmed; immediately after the first non-fork child spawns, `agent_runner` forwards that known id through `on_session_id` so `Session_store.set_session_id` durably marks it resumable before the turn can fail. If a stale unconfirmed session collides with an id Claude already claimed, `agent_process` retries once with `--resume`. Native forks stay unconfirmed until Claude emits the forked id because their placeholder id is never passed to the CLI.
